...
(function () {
  ...
  const defaultTableToolbar = 'tableSetting tabledelete | tablemergecells tablesplitcells | tableinsertrowbefore tableinsertrowafter tabledeleterow | tableinsertcolbefore tableinsertcolafter tabledeletecol'
  ...
  const registerCommands = editor => {
    ...
    editor.addCommand('mceTableInitSetting', (_, cbk) => {
      if (!isInit) {
        editor.addCommand('mceTableSetting', cbk)
      }
      isInit = true
    })
    editor.addCommand('mceTableSetting')
    // 提交表格设置
    editor.addCommand('mceTableSettingSubmit', (_, d) => {
      const dom = editor.dom
      let tableElm
      let data = extractDataFromSettings(editor, hasAdvancedTableTab(editor))
      tableElm = dom.getParent(editor.selection.getStart(), 'table', editor.getBody())
      ...
      // 提交
      onSubmitTableForm(editor, tableElm, data, {
        getData: () => newData,
        close: () => {}
      })
    })
    // 获取设置
    editor.editorCommands.addQueryValueHandler('mceTableSettingGet', () => {
      const dom = editor.dom
      let tableElm
      let data = extractDataFromSettings(editor, hasAdvancedTableTab(editor))
      tableElm = dom.getParent(editor.selection.getStart(), 'table', editor.getBody())
  ...
  const addButtons = (editor, selectionTargets) => {
    ...
    addButtonIfRegistered('tableSetting', {
      tooltip: 'Setting',
      command: 'mceTableSetting',
      icon: 'table-setting',
      onSetup: () => {}
    })
